...
deepspeed --include localhost:4,5,6 --master_port 29601 llava/train/run.py \
    ...
    --previous_task_model_path "${BASE_DIR}"/output/ucit/Task3_llava_lora_ours \
    --version $PROMPT_VERSION \
    --data_path "${BASE_DIR}"/instructions/IconQA/train.json \
    ...
    --bf16 False \
    --output_dir "${BASE_DIR}"/output/ucit/Task4_llava_lora_ours \
    --num_train_epochs 1 \
