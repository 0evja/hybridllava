...
deepspeed --include localhost:4,5,6 --master_port 29601 llava/train/run.py \
    --deepspeed ./scripts/zero2.json \
    --lora_enable True --lora_r 48 --lora_alpha 96 --mm_projector_lr 2e-5 \
    --expert_num 6 \
    --num_tasks 6 \
    --model_name_or_path "${BASE_DIR}"/models/llava-v1.5-7b \
    --previous_task_model_path "${BASE_DIR}"/output/ucit/Task2_llava_lora_ours \
    --version $PROMPT_VERSION \
    --data_path "${BASE_DIR}"/instructions/VizWiz/train.json \
    ...
    --bf16 False \
    --output_dir "${BASE_DIR}"/output/ucit/Task3_llava_lora_ours \
    --num_train_epochs 1 \
